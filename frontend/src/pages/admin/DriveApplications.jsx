...
export default function DriveApplications() {
  ...
  const fetchDriveDetails = async () => {
    try {
      const response = await authenticatedFetch(`http://localhost:5000/api/admin/drives`);
      if (response.ok) {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      const response = await authenticatedFetch(`http://localhost:5000/api/admin/drives/${driveId}/applications`);
  ...
  const handleMoveToNextStage = async (applicationId) => {
    ...
    try {
      ...
      const response = await authenticatedFetch(
        `http://localhost:5000/api/admin/applications/${applicationId}/next-stage`,
        {
          method: 'PUT'
        }
      );
