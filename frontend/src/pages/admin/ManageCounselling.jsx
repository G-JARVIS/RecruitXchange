...
export default function ManageCounselling() {
  ...
  const fetchSessions = async () => {
    try {
      setLoading(true);
      const response = await authenticatedFetch('http://localhost:5000/api/admin/counselling/sessions');
  ...
  const fetchApplications = async () => {
    try {
      const response = await authenticatedFetch('http://localhost:5000/api/admin/counselling/applications');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingSession 
        ? `http://localhost:5000/api/admin/counselling/sessions/${editingSession._id}`
        : 'http://localhost:5000/api/admin/counselling/sessions';
  ...
  const handleDelete = async (sessionId) => {
    ...
    try {
      const response = await authenticatedFetch(`http://localhost:5000/api/admin/counselling/sessions/${sessionId}`, {
        method: 'DELETE'
      });
  ...
  const handleUpdateApplicationStatus = async (applicationId, status, notes = '') => {
    try {
      const response = await authenticatedFetch(
        `http://localhost:5000/api/admin/counselling/applications/${applicationId}`,
        {
          method: 'PUT',
