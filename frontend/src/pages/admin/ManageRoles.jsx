...
export default function ManageRoles() {
  ...
  const fetchRoles = async () => {
    try {
      setLoading(true);
      const response = await authenticatedFetch('http://localhost:5000/api/admin/roles');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editingRole 
        ? `http://localhost:5000/api/admin/roles/${editingRole._id}`
  ...
  const handleDelete = async (roleId) => {
    ...
    try {
      const response = await authenticatedFetch(`http://localhost:5000/api/admin/roles/${roleId}`, {
        method: 'DELETE'
      });
