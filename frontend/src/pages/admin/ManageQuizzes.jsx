...
export default function ManageQuizzes() {
  ...
  const fetchQuizzes = async () => {
    try {
      setLoading(true);
      const response = await authenticatedFetch('http://localhost:5000/api/admin/quizzes');
  ...
  const handleDelete = async (quizId) => {
    ...
    try {
      const response = await authenticatedFetch(`http://localhost:5000/api/admin/quizzes/${quizId}`, {
        method: 'DELETE'
      });
