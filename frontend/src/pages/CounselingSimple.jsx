...
export default function CounselingSimple() {
  ...
  const fetchSessions = async () => {
    try {
      const response = await authenticatedFetch('http://localhost:5000/api/counseling/sessions');
      if (response.ok) {
  ...
  const fetchMyApplications = async () => {
    try {
      const response = await authenticatedFetch('http://localhost:5000/api/counseling/my-applications');
      if (response.ok) {
        const data = await response.json();
  ...
  const handleApplyForSession = async () => {
    ...
    try {
      const response = await authenticatedFetch(
        `http://localhost:5000/api/counseling/sessions/${selectedSession._id}/apply`,
        {
          method: 'POST',
